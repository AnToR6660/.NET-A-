...
namespace ZeroHungerV2.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        {
            var data = RestaurantController.Convert(e.user);
            data.Uname = e.name;
            db.users.Add(RestaurantController.Convert(data));
            db.SaveChanges();
        ...
        public ActionResult EmployeeFood()
        {
            ...
            var filteredData = db.Approveds.Where(f => f.Employee.uid == user.id && f.Food.status=="OnDelivery").ToList();
        ...
        public ActionResult CollectedFood() 
        {
            ...
            var filteredData = db.Approveds.Where(f => f.Employee.uid == user.id && f.Food.status == "Collected").ToList();
            ViewBag.emp = (user)Session["User"];
            return View(ApprovedController.Convert(filteredData));
        ...
        [HttpPost]
        public ActionResult CollectedFood(ApprovedDTO a)
        {
            var food = (from u in db.Foods
                        where u.id.Equals(a.Fid)
                        select u).SingleOrDefault();
            ...
            aprv.deliveredtime = data.deliveredtime;
            db.SaveChanges();
            return RedirectToAction("CollectedFood");

        }


        public ActionResult DeliveredFood()
        {
            var user = (user)Session["User"];
